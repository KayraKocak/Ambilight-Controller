...
namespace AmbilightControllerForm
{
    public partial class Form1 : Form
    {
        ...
        private GlassButton btnOnOff;
        private GlassButton btnToggleAmbilight;
        private GlassButton btnRainbow;
        ...
        private GlassButton btnModeToggle;
        ...
        private GlassButton btnAddressableSettings;
        ...
        private void UpdatePresetButtonState(GlassButton btn)
        {
        ...
        private void InitializeCustomComponents()
        {
            ...
            btnAddressableSettings = new GlassButton
            {
            ...
            btnAddPreset.Click += (s, e) => {
                ...
                // Find first empty preset GlassButton
                GlassButton firstEmpty = null;
                foreach (Control ctrl in presetsFlow.Controls)
                {
                    if (ctrl is GlassButton btn)
                    {
                ...
                GlassButton targetBtn = firstEmpty;
                if (targetBtn == null && presetsFlow.Controls.Count > 0)
                {
                    targetBtn = presetsFlow.Controls[presetsFlow.Controls.Count - 1] as GlassButton;
                }
            ...
            itemSetToCurrent.Click += (s2, e2) => {
                if (presetMenu.SourceControl is GlassButton btn)
                {
                    btn.Tag = null;
            ...
            MouseEventHandler presetMouseMove = (s2, e2) => {
                ...
                {
                    GlassButton btn = (GlassButton)s2;
                    Color c = btn.BackColor;
            ...
            DragEventHandler presetDragEnter = (s2, e2) => {
                if (e2.Data != null && e2.Data.GetDataPresent(typeof(GlassButton)))
                {
            ...
            DragEventHandler presetDragDrop = (s2, e2) => {
                ...
                GlassButton targetBtn = (GlassButton)s2;
                GlassButton sourceBtn = (GlassButton)e2.Data.GetData(typeof(GlassButton));
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                GlassButton presetBtn = new GlassButton
                {
                    Width = 40,
                    Height = 30,
                    BackColor = presetCol,
        ...
        private void ApplyAddressableActionCore(int index, MouseButtons GlassButton)
        {
            if (GlassButton == MouseButtons.Left)
            {
                if (isSelecting) selectedSegments.Add(index);
                else selectedSegments.Remove(index);
            }
            else if (GlassButton == MouseButtons.Right)
            {
                segmentColors[index] = Color.Black;
                selectedSegments.Remove(index);
            }
            else if (GlassButton == MouseButtons.Middle)
            {
                int leftIndex = -1;
        ...
        // ==========================================================================
        // GlassButton ACTIONS EVENT HANDLERS
        // ==========================================================================
        ...
        private GlassButton CreateDashboardButton(string mainText, string subText, int x, int y, bool preActive, int width = 256)
        {
        ...
        private GlassButton CreateTitleBarButton(string text, int x, Color hoverColor)
        {
            GlassButton btn = new GlassButton
            {
                Text = text,
